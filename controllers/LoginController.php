...
class LoginController extends Controller
{
    ...
    * *** ************************** */
    ...
    /**
     * Envia token para gera��o de nova senha por email.
     */
...
//		$url = "http://sistemas.icomp.ufam.edu.br/novoppgi/backend/web/index.php?r=afastamentos%2Fprint&id=".$model->id;
        ...
        try{
               Yii::$app->mailer->compose()
                ...
                ->setTo($model->email)
                ->setSubject($subject)
                ->setHtmlBody($this->renderPartial('email', ['usuario' => $model->name, 'url' => $url]), [])
                ->send();
				
        }catch(Exception $e){
                $this->mensagens('warning', 'Erro ao enviar Email(s)', 'Ocorreu um Erro ao Enviar Recupera��o de Senha.
